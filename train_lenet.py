...
@Time    : 2020/6/10 1:15
@Author  : HHNa
@FileName: train_lenet.py
@Software: PyCharm
# @brief      : cat vs dog分类模型训练
...
for epoch in range(MAX_EPOCH):
    ...
    for i, data in enumerate(train_loader):
        ...
        # forward
        inputs, labels = data
        # inputs = inputs.to("cuda") # 使用cuda
        # labls = labels.to("cuda") # 使用cuda
        outputs = net(inputs)
...
torch.save(net, "./sava_model/1")

# ============================ inference ============================
# net = torch.load("./sava_model/1")
